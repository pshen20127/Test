...
#include "CrystalPlasticityStressUpdateBaseAddStateVars.h"
...
InputParameters
CrystalPlasticityStressUpdateBaseAddStateVars::validParams()
{
...
CrystalPlasticityStressUpdateBaseAddStateVars::CrystalPlasticityStressUpdateBaseAddStateVars(
  ...
  : Material(parameters),
    ...
    _shearstrain(declareProperty<std::vector<Real>>(_base_name + "shearstrain")),
    _shearstrain_old(getMaterialPropertyOld<std::vector<Real>>(_base_name + "shearstrain")),
    equivalent_shearstrain(declareProperty<RankTwoTensor>("equivalent_shearstrain")),
    equivalent_shearstrain_old(getMaterialPropertyOld<RankTwoTensor>("equivalent_shearstrain")),
    equivalent_plastic_strain(declareProperty<Real>("equivalent_plastic_strain")),
    schmid_factor(declareProperty<std::vector<Real>>("schmid_factor")),
...
void
CrystalPlasticityStressUpdateBaseAddStateVars::initQpStatefulProperties()
{
  ...
  _shearstrain[_qp].resize(_number_slip_systems);
  schmid_factor[_qp].resize(_number_slip_systems);
  equivalent_shearstrain[_qp].zero();
}

void
CrystalPlasticityStressUpdateBaseAddStateVars::getSlipSystems()
{
  ...
  if (orthonormal_error)
    mooseError("CrystalPlasticityStressUpdateBaseAddStateVars Error: The slip system file contains a slip "
               "direction and plane normal pair that are not orthonormal in the Cartesian "
               "coordinate system.");
}

void
CrystalPlasticityStressUpdateBaseAddStateVars::transformHexagonalMillerBravaisSlipSystems(
    const MooseUtils::DelimitedFileReader & reader)
{
  ...
  if (_unit_cell_dimension[0] != _unit_cell_dimension[1] ||
    ...
    mooseError("CrystalPlasticityStressUpdateBaseAddStateVars Error: The specified unit cell dimensions are "
               "not consistent with expectations for "
               "HCP crystal hexagonal lattices.");
  else if (reader.getData(0).size() != miller_bravais_indices * 2)
    mooseError("CrystalPlasticityStressUpdateBaseAddStateVars Error: The number of entries in the first row of "
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    for (const auto j : index_range(reader.getData(i)))
    {
      ...
      if (basal_pl_sum > _zero_tol)
        mooseError(
            "CrystalPlasticityStressUpdateBaseAddStateVars Error: The specified HCP basal plane Miller-Bravais "
      ...
      if (basal_dir_sum > _zero_tol)
        mooseError("CrystalPlasticityStressUpdateBaseAddStateVars Error: The specified HCP slip direction "
...
void
CrystalPlasticityStressUpdateBaseAddStateVars::sortCrossSlipFamilies()
{
...
unsigned int
CrystalPlasticityStressUpdateBaseAddStateVars::identifyCrossSlipFamily(const unsigned int index)
{
  for (unsigned int i = 0; i < _number_cross_slip_directions; ++i)
    for (unsigned int j = 0; j < _number_cross_slip_planes; ++j)
...
void
CrystalPlasticityStressUpdateBaseAddStateVars::calculateFlowDirection(const RankTwoTensor & crysrot)
{
...
void
CrystalPlasticityStressUpdateBaseAddStateVars::calculateSchmidTensor(
    ...
    const RankTwoTensor & crysrot)
{
  ...
  std::vector<Real> cosinePhi, cosineLamda;
  cosinePhi.resize(number_slip_systems);
  cosineLamda.resize(number_slip_systems);
/*
  FILE *fp = fopen("/data/pshen/IN625/test/output.txt","w");
  fprintf(fp, "plane_normal_vector[0](0) = %f\n",plane_normal_vector[0](0));
  fprintf(fp, "plane_normal_vector[0](1) = %f\n",plane_normal_vector[0](1));
  fprintf(fp, "plane_normal_vector[0](2) = %f\n",plane_normal_vector[0](2));
  fprintf(fp, "direction_vector[0](0) = %f\n",direction_vector[0](0));
  fprintf(fp, "direction_vector[0](1) = %f\n",direction_vector[0](1));
  fprintf(fp, "direction_vector[0](2) = %f\n",direction_vector[0](2));
  fprintf(fp, "plane_normal_vector[3](0) = %f\n",plane_normal_vector[3](0));
  fprintf(fp, "plane_normal_vector[3](1) = %f\n",plane_normal_vector[3](1));
  fprintf(fp, "plane_normal_vector[3](2) = %f\n",plane_normal_vector[3](2));
  fprintf(fp, "direction_vector[3](0) = %f\n",direction_vector[3](0));
  fprintf(fp, "direction_vector[3](1) = %f\n",direction_vector[3](1));
  fprintf(fp, "direction_vector[3](2) = %f\n",direction_vector[3](2));
  fprintf(fp, "plane_normal_vector[8](0) = %f\n",plane_normal_vector[8](0));
  fprintf(fp, "plane_normal_vector[8](1) = %f\n",plane_normal_vector[8](1));
  fprintf(fp, "plane_normal_vector[8](2) = %f\n",plane_normal_vector[8](2));
  fprintf(fp, "direction_vector[8](0) = %f\n",direction_vector[8](0));
  fprintf(fp, "direction_vector[8](1) = %f\n",direction_vector[8](1));
  fprintf(fp, "direction_vector[8](2) = %f\n",direction_vector[8](2));
*/
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    // Calculate schmid factor(loading direction is (1,0,0) in this case)
    cosinePhi[i] = local_direction_vector[i](0) / sqrt(pow(local_direction_vector[i](0),2)+pow(local_direction_vector[i](1),2)+pow(local_direction_vector[i](2),2));
    cosineLamda[i] = local_plane_normal[i](0) / sqrt(pow(local_plane_normal[i](0),2)+pow(local_plane_normal[i](1),2)+pow(local_plane_normal[i](2),2));
    schmid_factor[_qp][i] = cosinePhi[i] * cosineLamda[i];
    //printf("cosinePhi%d = %f\n",i,cosinePhi[i]);
    //printf("cosineLamda%d = %f\n",i,cosineLamda[i]);
    //printf("schmid_factor%d = %f\n",i,schmid_factor[_qp][i]);
  }

/*
  fprintf(fp, "local_plane_normal[0](0) = %f\n",local_plane_normal[0](0));
  fprintf(fp, "local_plane_normal[0](1) = %f\n",local_plane_normal[0](1));
  fprintf(fp, "local_plane_normal[0](2) = %f\n",local_plane_normal[0](2));
  fprintf(fp, "local_direction_vector[0](0) = %f\n",local_direction_vector[0](0));
  fprintf(fp, "local_direction_vector[0](1) = %f\n",local_direction_vector[0](1));
  fprintf(fp, "local_direction_vector[0](2) = %f\n",local_direction_vector[0](2));
  fprintf(fp, "local_plane_normal[3](0) = %f\n",local_plane_normal[3](0));
  fprintf(fp, "local_plane_normal[3](1) = %f\n",local_plane_normal[3](1));
  fprintf(fp, "local_plane_normal[3](2) = %f\n",local_plane_normal[3](2));
  fprintf(fp, "local_direction_vector[3](0) = %f\n",local_direction_vector[3](0));
  fprintf(fp, "local_direction_vector[3](1) = %f\n",local_direction_vector[3](1));
  fprintf(fp, "local_direction_vector[3](2) = %f\n",local_direction_vector[3](2));
  fprintf(fp, "local_plane_normal[8](0) = %f\n",local_plane_normal[8](0));
  fprintf(fp, "local_plane_normal[8](1) = %f\n",local_plane_normal[8](1));
  fprintf(fp, "local_plane_normal[8](2) = %f\n",local_plane_normal[8](2));
  fprintf(fp, "local_direction_vector[8](0) = %f\n",local_direction_vector[8](0));
  fprintf(fp, "local_direction_vector[8](1) = %f\n",local_direction_vector[8](1));
  fprintf(fp, "local_direction_vector[8](2) = %f\n",local_direction_vector[8](2));
  fclose(fp);
*/
}

void
CrystalPlasticityStressUpdateBaseAddStateVars::calculateShearStress(
...
void
CrystalPlasticityStressUpdateBaseAddStateVars::calculateTotalPlasticDeformationGradientDerivative(
...
void
CrystalPlasticityStressUpdateBaseAddStateVars::calculateEquivalentSlipIncrement(
    RankTwoTensor & equivalent_slip_increment)
{
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    //printf("_flow_direction%d = %f\n",i,_flow_direction[_qp][i]);
    //printf("slip system %d : \n",i);
  ...
  equivalent_shearstrain[_qp] = equivalent_slip_increment;
  equivalent_shearstrain[_qp] += equivalent_shearstrain_old[_qp];
  equivalent_plastic_strain[_qp] = sqrt(pow(equivalent_shearstrain[_qp](0,0),2)+pow(equivalent_shearstrain[_qp](0,1),2)+pow(equivalent_shearstrain[_qp](0,2),2)
                                       +pow(equivalent_shearstrain[_qp](1,0),2)+pow(equivalent_shearstrain[_qp](1,1),2)+pow(equivalent_shearstrain[_qp](1,2),2)
                                       +pow(equivalent_shearstrain[_qp](2,0),2)+pow(equivalent_shearstrain[_qp](2,1),2)+pow(equivalent_shearstrain[_qp](2,2),2));
}


void
CrystalPlasticityStressUpdateBaseAddStateVars::setQp(const unsigned int & qp)
{
  _qp = qp;
}

void
CrystalPlasticityStressUpdateBaseAddStateVars::setSubstepDt(const Real & substep_dt)
{
  _substep_dt = substep_dt;
}

bool
CrystalPlasticityStressUpdateBaseAddStateVars::isConstitutiveStateVariableConverged(
