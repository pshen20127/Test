...
class CrystalPlasticityStressUpdateBaseAddStateVars : public Material
{
public:
  static InputParameters validParams();

  CrystalPlasticityStressUpdateBaseAddStateVars(const InputParameters & parameters);
...
protected:
  ...
  MaterialProperty<std::vector<Real>> & _shearstrain;
  const MaterialProperty<std::vector<Real>> & _shearstrain_old;

  MaterialProperty<RankTwoTensor> & equivalent_shearstrain;
  const MaterialProperty<RankTwoTensor> & equivalent_shearstrain_old;

  MaterialProperty<Real> & equivalent_plastic_strain;

  /// Schmid factor
  MaterialProperty<std::vector<Real>> & schmid_factor;
