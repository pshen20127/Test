...
#include "CrystalPlasticityKalidindiUpdateAddStateVars.h"
#include "libmesh/int_range.h"
//#include <stdio.h>
//#include <stdlib.h>

//FILE *fp = fopen("/home/pshen/Mercury2/IN625/test/shearstrain.txt","a+");

registerMooseObject("TensorMechanicsApp", CrystalPlasticityKalidindiUpdateAddStateVars);

InputParameters
CrystalPlasticityKalidindiUpdateAddStateVars::validParams()
{
  InputParameters params = CrystalPlasticityStressUpdateBaseAddStateVars::validParams();
...
CrystalPlasticityKalidindiUpdateAddStateVars::CrystalPlasticityKalidindiUpdateAddStateVars(
    const InputParameters & parameters)
  : CrystalPlasticityStressUpdateBaseAddStateVars(parameters),
    // Constitutive values
    _r(getParam<Real>("r")),
    _h(getParam<Real>("h")),
...
void
CrystalPlasticityKalidindiUpdateAddStateVars::initQpStatefulProperties()
{
...
//    _shearstrain[_qp][i] = 0.0;
  }
}

void
CrystalPlasticityKalidindiUpdateAddStateVars::setInitialConstitutiveVariableValues()
{
...
void
CrystalPlasticityKalidindiUpdateAddStateVars::setSubstepConstitutiveVariableValues()
{
...
bool
CrystalPlasticityKalidindiUpdateAddStateVars::calculateSlipRate()
{
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    _shearstrain[_qp][i] = std::abs(_slip_increment[_qp][i]) * _substep_dt;
    //printf("slip_increment%d = %f\n",i,_slip_increment[_qp][i]);
    //fprintf(fp, "shearstrain%d = %lf\n",i,_shearstrain[_qp][i]);
...
void
CrystalPlasticityKalidindiUpdateAddStateVars::calculateEquivalentSlipIncrement(
    RankTwoTensor & equivalent_slip_increment)
{
  if (_include_twinning_in_Lp)
  {
    ...
    CrystalPlasticityStressUpdateBaseAddStateVars::calculateEquivalentSlipIncrement(equivalent_slip_increment);
}

void
CrystalPlasticityKalidindiUpdateAddStateVars::calculateConstitutiveSlipDerivative(
...
bool
CrystalPlasticityKalidindiUpdateAddStateVars::areConstitutiveStateVariablesConverged()
{
...
void
CrystalPlasticityKalidindiUpdateAddStateVars::updateSubstepConstitutiveVariableValues()
{
  // Would also set substepped dislocation densities here if included in this model
...
void
CrystalPlasticityKalidindiUpdateAddStateVars::cacheStateVariablesBeforeUpdate()
{
  _slip_resistance_before_update = _slip_resistance[_qp];
}

void
CrystalPlasticityKalidindiUpdateAddStateVars::calculateStateVariableEvolutionRateComponent()
{
  for (const auto i : make_range(_number_slip_systems))
  {
...
bool
CrystalPlasticityKalidindiUpdateAddStateVars::updateStateVariables()
{
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    _shearstrain[_qp][i] += _shearstrain_old[_qp][i];
  }
  return true;
}
